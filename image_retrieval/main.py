...
@log_all_methods(ignore=["register"])
class ImageRetriever:
    ...
    def register(self, doc_id, point_id, r):
        ...
        """
        hindex = calc_index(r, self.k, self.max_size)
    ...
    def index(self, img, doc_id):
        """
        calculate and store features for an image in the database
        """
        ...
        if n < 0 or m < 0:
            # TODO: check if this is an impossible case and raise exception
            return None

        for p in feature_point:
            npoints = nearest_points(feature_point, p, n)
            for mask in combinations(np.arange(n), m):
                r = calc_invariant(npoints[list(mask)], self.invariant)
                self.register(doc_id, p, r)
        ...
        for p in feature_point:
            ...
            for mask in combinations(np.arange(n), m):
                ...
                for _ in range(m):
                    ...
                    r = calc_invariant(mpoints, self.invariant)
                    hindex = calc_index(r, self.k, self.max_size)
...
if __name__ == "__main__":
    ...
    for idx, f in enumerate(files):
        i.index(imread(f.name, mode=0), idx)

    print(i.query(imread(files[0].name, mode=0)))
